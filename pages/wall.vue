...
<script>
  ...
  export default {
    ...
    fetch ({ store }) {
      return store.dispatch('REQ_wallList', {
        page:1,
        page_size: 10
      })
    },
    ...
    methods: {
      ...
      loadMore () {
        this.$store.dispatch('REQ_wallList', {
          page: this.$store.state.heros.data.pagination.current_page + 1
        })
      },
      ...
      async submit () {
        ...
        const res = await this.$store.dispatch('REQ_addWall', { ...this.form })
