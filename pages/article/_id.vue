...
export default {
  ...
  fetch({ store , params }) {
    return store.dispatch('REQ_getArtDetails', params.id)
  },
  ...
  methods: {
    async like () {
      ...
      const res = await this.$store.dispatch('REQ_likeArticle', this.article._id)
      if (res.code !== 1) alert(`喜欢文章失败：${res.message}`)
      else {
        let data = JSON.parse(JSON.stringify(this.article))
        data.meta.likes += 1
        this.$store.commit('article/SET_DETAILS', data)
        this.likeArticles.push(data._id)
