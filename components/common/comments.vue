        ...
        <div class="user">
          <div class="gravatar" v-if="!mobileLayout">
            <img :alt="user.name || '匿名用户'"
                 :src="user.gravatar || '/images/logo.png'">
          </div>
        </div>
        <div class="editor">
...
export default {
  ...
  computed: {
    ...
    haveMore () {
      return this.$store.state.comment.data.pagination.page
  ...
  methods: {
    ...
    async loadComemntList(params) {
      ...
      const res = await this.$store.dispatch('REQ_commits', {
        ...params,
        post_id: this.postId
      })
    },
    ...
    likeComment(comment) {
      ...
      this.$store.dispatch('REQ_likeCommit', comment._id)
      .then(data => {
    ...
    async submitComment(event) {
      ...
      const res = await this.$store.dispatch('REQ_addCommit', {
        pid: this.pid,
        post_id: this.postId,
  ...
  mounted () {
    ...
    if (!!this.mobileLayout) {
      this.commentForm.page_size = 100
      this.loadComemntList(this.commentForm)
      return
    }
    ...
    window.onscroll = _.throttle(() => {
      ...
      if (scrollHeight -  scrolleTop - windowHeight <= 200) {
        if (!this.comment.data.pagination.total_page && !this.comment.fetching) {
          ...
          this.commentForm.page = this.comment.data.pagination.page + 1
          this.loadComemntList(this.commentForm)
        }
      }
    }, 400)
